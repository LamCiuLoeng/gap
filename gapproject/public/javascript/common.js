...
var selectAllDetail = function(obj,selectBox){$(obj).attr("checked") ? $("."+selectBox+" :checkbox").attr("checked","checked") :  $("."+selectBox+" :checkbox").removeAttr("checked");}
...
var showError = function(msg){
    ...
    $.prompt(msg,{opacity: 0.6,prefix:'cleanred',top:'40%',left:'35%'});
}
var showMsg = function(msg){
    ...
    $.prompt(msg,{opacity: 0.6,prefix:'cleanblue',top:'40%',left:'35%',useiframe:true});
}
...
var showWaiting = function(){$.blockUI({message: '<img src="../images/busy.gif" /> loading...',css:{width:'150px',padding:'5px 0',fontSize:'16px',fontWeight:'400'}})}
...
var changeVal = function(obj){
	...
	if(obj.attr("tax_rate")>=0){
		if (obj.attr("include_tax") == 0){
			unit_price = new Number(obj.attr('unit_price'))
		...
		{	
			unit_price = obj.attr('unit_price')* (parseFloat(obj.attr("tax_rate"))+1)
			total_amount = $("#qty_"+ids).html() * unit_price
			total_amount_without_tax = $("#unit_price_tax_"+ids).html()*$("#qty_"+ids).html()
			tax = total_amount - total_amount_without_tax 
			total_amount_total_amount = total_amount
