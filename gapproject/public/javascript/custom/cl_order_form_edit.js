...
function toAdd(){
	...
	$("input",tr).each(function(){
		...
		t.attr("name",n);
		if (t.attr("name").split("_")[0] == "clitem") {
...
function confirmOrder(){
	...
	if( $("#shipCompany").val() == '-1' ){msg.push("* Please select the 'Ship To' company!");}
	if( !$("#shipAddress").val() ){msg.push("* Please input the 'Ship To' address!");}
	if( !$("#styleNo").val() ){msg.push("* Please input the 'Style No#'!");}
	if( !$("#styleName").val() ){msg.push("* Please input the 'Style Name'!");}
	if( $("#billCompany").val() == '-1' ){msg.push("* Please select the 'Bill To' company!");}
	...
	$(".gridTable tbody tr").not('.template').each(function(){
	   var tmp = $(this);
	   var no = $("select[name^='clitem_']",tmp).val();
	   ...
	   if( no != '0' ){
           ...
           }else if (jQuery.inArray(no, item_index_arr) != -1 && $("select[name^='clitem_']").attr('name').split('_')[1] != 'x'){
	   ...
	   if($("select[name^='clsize_']", tmp).val() == '0') {
	   		isSize = false;
	   }
	   
	   if($("select[name^='artdesc_']", tmp).val() == '0') {
	   		isArtDesc = false;
	   }
    ...
    if(!isSize){ msg.push("* Please select the size!"); }
    if(!isArtDesc){msg.push("* Please select the Article Description!"); }	
...
function bindAutoComplete(selector){
    selector.each(function(){
        ...
        jqObj.autocomplete("/carelabel/getAjaxField", {
            ...
            jqObj.parent().parent().find("td:nth-child(4)").text(data[1]);
            jqObj.val(formatted);
        });      
    });
}