...
<div class="nav-tree">GAP&nbsp;&nbsp;&gt;&nbsp;&nbsp;Order Search Form</div>
	...
	<table class="gridTable" cellpadding="0" cellspacing="0" border="0">
		<%
			label_attrs = [ ('r-pac Confirmation No', 220),
							('Vendor PO#',"220"), ('Order Date',"200"),
							('Ship Instruction', '250'), ('Total Qty', 150),
							('Issued By', 150), ('Region', 200)
							]
		...
		<tbody>
			...
			%endif
				<td>
				<a href="/order/view?code=${rpacEncrypt(item.id)}">
				${item.orderNO|b}
				</a>
				<td>${item.vendorPO|b}</td>
				<td>
				${Date2Text(item.orderDate)|b}
				</td>
