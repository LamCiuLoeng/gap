...
<div class="nav-tree">Reserve&nbsp;&nbsp;:&nbsp;&nbsp;</div>
<img src="/images/search_10.jpg" width="600" height="2" />
<div style="clear:both"><br/></div>
% if details:
<form id="re-save-form" action="/ship/saveReserve" method="post">
...
<table cellspacing="0" cellpadding="0" border="0" width="760" class="gridTable">
    ...
    <tr>
        ...
        <td align="center" width="160" class="wt-td">Reserve Qty</td>
    </tr>
    </thead>
    <tbody>
    ...
    %endif
    <%
        from gapproject.util.inventory_helper import reserveMinMax
        qty = d.qty
        qtyReserved = d.qtyReserved
        qtyShipped = d.qtyShipped
        availableQty = d.item.availableQtyByWarehouse(warehouse.id)
        minQty, maxQty = reserveMinMax(qty, qtyReserved, availableQty)
    %>
        ...
        <td align="center" class="t-td">
            % if qtyReserved == qty or availableQty < 1:
            &nbsp;
            % else:
            <input type="text" name="qty-${d.id}" min="${minQty}" max='${maxQty}' class="required numeric digits"/>
            % endif
        </td>
    </tr>
    %endfor
